...
int bmh_scan_v2(const std::string& t, const std::string& p) {
    ...
    while (cursor <= N - M) {
        ...
        cursor += table[(unsigned char)t[cursor]]; //[cursor + m - 1]
    }
    return -1;
}
...
string decodePrefix(const string& cipher, int startR1, int startR2, int K) {
    ...
    for (int i = 0; i < (int)cipher.size() && count < K; i++) {
        char ch = cipher[i];
    ...
    for (int r1 = 0; r1 < 95; r1++) {
        for (int r2 = 0; r2 < 95; r2++) {
            ...
            if (scoreV1 >= 6) {
                ...
                fout << "Rotor positions:\n";
                fout << "r1 = " << r1 << ", r2 = " << r2 <<"\n\n";

                fout << "Anchor words matched:\n";
                for(const string& word : anchors){
                    if(bmh_findV1(prefix, word) != -1){
                        fout << word << " ";
                    }
                ...
                fout << "First 200 characters of decoded prefix:\n";
                fout << prefix.substr(0, 200) << "\n\n";

                fout << "Correct BMH variant: V1\n\n";

                fout << "Why others fail:\n";
                fout << "V2 should be [cursor + m - 1], so it jumps wrong and misses at index 23\n";
                fout << "Version 3(horspool_match_v3), the for loop is i < m where it should be i < m -1. This is why it is in an infinite loop and will not give the output.\n";

                fout << "Full decoded ciphertext:\n";
                fout << fullDecoded << "\n";
