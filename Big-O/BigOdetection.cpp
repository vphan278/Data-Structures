...
vector<string> tokenize(const string& code) {
    string spaced;
    for (char c : code) {                             // Goes character by character through the input string.
        if (ispunct((unsigned char)c)) {              // If the character is punctuation (( ) ; { } + - / < > etc.)
            spaced += ' ';                            // It adds spaces before and after it.
            spaced += c;                              // It lets your Big-O detector analyze code token by token instead of raw text.
            spaced += ' ';
        } 
        else {
            spaced += c;
...
string BigOdetection(string code) {
    ...
    // Part 2                                               // It scans the raw code string using string.find() to detect certain keywords.
    bool hasFor = (code.find("for") != string::npos);       // It does zero structural analysis.
    ...
    /////////////////////////////////////////////////////////////////
    vector<string> tokens = tokenize(code);            // Turns raw code into tokens like  :  ["while","(","n",">","1",")","{","n","=","n","/","2",";","}"]
...
void printTokens(const string& code) {   // is a debugging/visualization tool
